...
*/
function getResults(){
    if (games.length != 0) alert('þú spilaðir ' + games.length + ' leiki\nMeðalfjöldi ágiskana var ' + calculateAverage());
    else alert('Þú spilaðir engann leik >_<');
}
...
function getResponse(guess, correct){
    if (guess < 0 || guess == null) alert('Ekki rétt');
    else if (guess == correct) alert('Rétt');
    else if (Math.abs(correct-guess) < 5) alert('Mjög nálægt');
    else if (Math.abs(correct-guess) < 10) alert('Nálægt');
    else if (Math.abs(correct-guess) < 20) alert('Frekar langt frá');
    else if (Math.abs(correct-guess) < 50) alert('Langt frá');
    else alert('Mjög langt frá');
}
